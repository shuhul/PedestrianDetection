from src.oldfiles.ImageHandler import getImgsHSB
from src.oldfiles.ImageHandler import getImgsGS
import numpy as np
...
def GetTexture(imageGS):
    ...
    textures = [mean(contrasts), variance(contrasts), mean(correlations), variance(correlations), mean(energys),
                variance(energys), mean(entropys), variance(entropys)]
